...
export function transformLynxStyles(hypenNameStyles) {
    const newStyle = [];
    const childStyle = [];
    for (const declaration of hypenNameStyles) {
        ...
        if (rule?.[value]) {
            // remove the current
            for (const [newProperty, newValue] of rule[value]) {
            ...
            const { flexGrow, flexShrink, flexBasis } = parseFlexShorthand(value);
            newStyle.push([
                '--flex-grow',
                flexGrow,
            ], [
...
//# sourceMappingURL=transformLynxStyles.js.map