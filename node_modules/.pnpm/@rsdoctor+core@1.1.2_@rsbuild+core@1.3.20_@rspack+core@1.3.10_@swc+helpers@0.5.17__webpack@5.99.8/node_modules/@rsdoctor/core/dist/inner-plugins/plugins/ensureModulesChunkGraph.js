...
const ensureModulesChunksGraphFn = (compiler, _this) => {
  ...
  const enableRspackNativePlugin = _this.options.experiments?.enableNativePlugin;
  if (enableRspackNativePlugin && RsdoctorRspackPlugin) {
    (0, import_rspack.applyRspackNativePlugin)(compiler, _this, RsdoctorRspackPlugin);
  }
  compiler.hooks.done.tapPromise(
    ...
    async (_stats) => {
      ...
      if (!_this.chunkGraph?.getChunks().length) {
        _this.chunkGraph = import_build2.Chunks.chunkTransform(
          /* @__PURE__ */ new Map(),
          getStatsJson()
        );
      }
